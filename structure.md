...
- **`Agent` class** — The central orchestrator. Holds conversation context, system prompt, LLM backend, and budget.
  ...
  - `save_context()` / `load_context()` — Pause/resume of full conversation state including token counts, costs, the session metrics rollup, and the step count (so a resumed session keeps the whole-task step number, tokens/rate, and cost-per-hour).
